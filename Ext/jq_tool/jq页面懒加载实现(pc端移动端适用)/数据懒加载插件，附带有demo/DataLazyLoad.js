...
(function ($, window, undefined) {
    $.fn.DataLazyLoad = function (options) {
        ...
        $(window).scroll(function () {
            ...
            var offset = $(elements).offset().top + $(elements).height() - (scrollTop + winHeight-200);//滚动条离底部的计算
    ...
    $.fn.DataLazyLoad = function (options) {
        ...
        $(window).scroll(function () {
            var scrollTop = $(window).scrollTop();
            //elements height + elements top - (scrollbar top + browser window height)
            var offset = $(elements).offset().top + $(elements).height() - (scrollTop + winHeight-200);//滚动条离底部的计算，最大可减到200
            if (offset < settings.offset && !locked) {
                locked = true;
